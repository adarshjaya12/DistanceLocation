...
def GetCoordinateList(latitude,longitude,distance):
	...
	adjustedDistance = float(distance)/radiusEarth
	myInter = iter(range(0,359))
	for i in myInter:
		lat2 = math.asin(math.sin(ConvertToRadians(latitude)) * math.cos(adjustedDistance) + math.cos(ConvertToRadians(latitude)) * math.sin(adjustedDistance) * math.cos(ConvertToRadians(i)))
